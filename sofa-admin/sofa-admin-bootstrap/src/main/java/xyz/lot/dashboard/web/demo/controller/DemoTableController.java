...
public class DemoTableController {
    ...
    @ResponseBody
    @AjaxWrapper
    public PageModel<UserTableModel> list(HttpServletRequest req, UserTableModel userModel) {
        ...
        long pageNum = (pageDomain.getPageNum()) - 1;
        ...
        int start = (int) (pageNum * pageSize);
        int end = start + (int) pageSize;
        if(end > userList.size())
            end = userList.size();

        return PageModel.instance(userList.size(), userList.subList(start, end));
    }
}
