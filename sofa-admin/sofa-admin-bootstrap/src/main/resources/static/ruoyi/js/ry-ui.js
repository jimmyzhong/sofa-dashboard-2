...
(function ($) {
    $.extend({
        ...
        treeTable: {
            ...
            responseHandler2: function(data) {
        ...
        validate: {
            // 判断返回标识是否唯一 false 不存在，可以使用 true 存在,不可以使用
            unique: function (value) {
        ...
        tree: {
            ...
            init: function (options) {
                ...
                $.get(options.url, function (data) {
                    ...
                    $._tree = tree;
                    for (var i = options.expandLevel; i > 0; i--) {
                        var nodes = tree.getNodesByParam("level", i - 1);
            ...
            // 获取当前被勾选集合 FULL
            getFullCheckedNodes: function (column) {
                function filter(node) {
                    return (!node.getCheckStatus().half && node.getCheckStatus().checked);
                }
                var _column = $.common.isEmpty(column) ? "id" : column;
                var nodes = $._tree.getNodesByFilter(filter);
                return $.map(nodes, function (row) {
                    return row[_column];
                }).join();
            },
